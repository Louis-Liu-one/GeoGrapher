...
BOOST_PYTHON_MODULE(GeoItems)
{
    ...
    class_<GeoItem>("GeoItem", "所有图元类的基类。",
        init<>(arg("self"), "初始化图元。"))
        .def("addMaster", &GeoItem::addMaster,
            (arg("self"), "master"),
    ...
    class_<GeoCircle, bases<GeoItem>>("GeoCircle", "圆图元类。",
        ...
        .def("onc", &GeoCircle::onc, arg("self"), "圆上一点。",
            return_value_policy<reference_existing_object>())
        .def("oandr", &GeoCircle::oandrPy,
            arg("self"), "圆心坐标和半径。")
        .def("r", &GeoCircle::r, arg("self"), "半径。");
    class_<GeoIntersection, bases<GeoPoint>>(
        "GeoIntersection", "交点图元类。",
        init<>(arg("self"), "初始化交点图元。"))
        .def("pos", &GeoIntersection::posPy,
            arg("self"), "交点坐标。");
    class_<GeoVariable<int>, bases<GeoItem>>("GeoIntVar",
        "整型变量图元类。", init<int>((arg("self"), "v"),
            "以整数初始化变量图元。"))
        .def("get", &GeoVariable<int>::get,
            arg("self"), "获取值。")
        .def("set", &GeoVariable<int>::set,
            (arg("self"), "v"), "设置值。");
    def("distanceTo", distanceTo_P,
        (arg("p1"), "p2"), "两点之间的距离。");
